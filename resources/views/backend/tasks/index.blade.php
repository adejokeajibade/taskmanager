...
@section('content')
    <div class="card">
        ...
        <div class="card-header">
            <a href="{{ route('task.new') }}" class="btn btn-success"><i class="fa fa-plus-circle"></i>&nbsp;&nbsp;{{ trans('redprint::core.new') }}</a>
            <div class="btn-group float-right">
                @if(count(Request::input()))
                    <a class="btn btn-default" href="{{ route('task.index') }}">{{ trans('redprint::core.clear') }}</a>
...
<td> {{ $taskItem->user_id }}</td>

                        <th>
                            @if(!$taskItem->deleted_at)
                                <a href="{{ route('task.form', $taskItem->id) }}" class="btn btn-primary btn-xs">{{ trans('redprint::core.edit') }}</a>
                                ...
                                <div class="modal fade" id="deleteModal{{ $taskItem->id }}">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <form class="form-horizontal" method="post" action="{{ route('task.delete', $taskItem->id) }}" >
                                            {!! csrf_field() !!}
                                            <div class="modal-header">
                                                <button type="button" class="close" data-dismiss="modal">&times;</button>
                                                <h4 class="modal-title"> {{ trans('redprint::core.delete') }}: {{ $taskItem->id }} </h4>
                                            </div>
                            
                                            <div class="modal-body">
                                                {{ trans('redprint::core.confirm_delete') }} <strong>{{ $taskItem->id }} ?</strong>
                                            </div>
    ...
    <div class="modal fade" id="searchModal">
        <div class="modal-dialog">
            <div class="modal-content">
                <form class="form-horizontal" method="get" action="{{ route('task.index') }}" >
                {!! csrf_field() !!}
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title">{{ trans('redprint::core.search') }} tasks</h4>
                </div>

                <div class="modal-body">                  
                    <div class="form-group">
					    
					    <label class="col-sm-3">{{ \Lang::has('redprint::strings.description') ? trans('redprint::strings.description') :  'Description' }} </label>
